...
class Runner(object):
    def __init__(self, opt, log, save_opt=True):
        ...
        self.x10net = Image256Net(log, noise_levels=noise_levels, use_fp16=opt.use_fp16, cond=None)
        self.x10ema = ExponentialMovingAverage(self.x10net.parameters(), decay=opt.ema)
        x10_path = os.path.join(opt.ckpt_path, "x10.pt")
        x10checkpoint = torch.load(x10_path, map_location="cpu")
        self.x10net.load_state_dict(x10checkpoint['net'])
        ...
        if opt.load:
            ...
            self.x10ema.load_state_dict(x10checkpoint["ema"])
            log.info(f"[x10Ema] Loaded ema ckpt: {x10_path}!")
        self.net.to(opt.device)
        self.ema.to(opt.device)
        self.x10net.to(opt.device)
        self.x10ema.to(opt.device)
    ...
    def train(self, opt, train_dataset, val_dataset, corrupt_method):
        ...
        x10net = DDP(self.x10net, device_ids=[opt.device])
        #x10ema = self.x10ema
        ...
        for it in range(opt.num_itr):
            ...
            for _ in range(n_inner_loop):
                ...
                x10 = x10net(x1, x10step, cond=cond).detach()

                # ===== compute loss =====
                step = torch.randint(1, opt.interval, (x0.shape[0],))
                xt ,label= self.diffusion.q_sample(step, x0, x10, ot_ode=opt.ot_ode)
    ...
    @torch.no_grad()
    def ddpm_sampling(self,img_clean, opt, x1, mask=None, cond=None, clip_denoise=False, nfe=None, d=d,log_count=10, verbose=True):
        ...
        x1 = x1.to(opt.device)
        if cond is not None: cond = cond.to(opt.device)# cond is not changed
        
        with self.ema.average_parameters() and self.x10ema.average_parameters():
            self.net.eval()
            self.x10net.eval()
            def x1_to_x10(x1):
                x10step = torch.full((x1.shape[0],), 1, device=opt.device, dtype=torch.long)
                x10 = self.x10net(x1, x10step, cond=cond)
                return x10
            def pred_drift(xt, step):
                step = torch.full((xt.shape[0],), step, device=opt.device, dtype=torch.long)
                out = self.net(xt, step, cond=cond)
                return out/2
            
            xs ,pred_x0= self.diffusion.ddpm_sampling(img_clean,
                steps, pred_drift,x1_to_x10, x1,d=d, ot_ode=opt.ot_ode, log_steps=log_steps, verbose=verbose,
            )
    ...
    def evaluation(self, opt, it, val_loader, corrupt_method):
        ...
        xs, pred_x0s = self.ddpm_sampling(img_clean,
