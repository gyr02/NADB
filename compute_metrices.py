...
def get_ref_fid(opt, log):
    ...
    split = "train1" if 'edges2handbags'  in ckpt_opt.corrupt or "edges2shoes" in ckpt_opt.corrupt else "val"#######################
   
    # build npz file
    if split == "train":
        ref_fid_fn = Path("/NADB/data/VIRTUAL_imagenet256_labeled.npz")
        ...
        ref_fid_fn = Path("/fid_imagenet_256_train.npz")
        if not ref_fid_fn.exists():
            ref_fid_fn=Path("/fid_imagenet_256_train.npz")
        ...
        ref_fid_fn = Path("/NADB/data/fid_imagenet_256_val.npz")
        if not ref_fid_fn.exists():
            ref_fid_fn=Path("/NADB/data/fid_imagenet_256_val.npz")
            log.info(f"Generating {ref_fid_fn=} (this can take a while ...)")
            ref_opt = build_ref_opt(opt, ref_fid_fn)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckpt",        type=str,  default="sr4x-pool",             help="the checkpoint name for which we wish to compute metrices")
    parser.add_argument("--mode",        type=str,  default="legacy_pytorch", help="the FID computation mode used in clean-fid")
    parser.add_argument("--dataset-dir", type=Path, default="/mnt/sdc/imagenet",       help="path to LMDB dataset")
    parser.add_argument("--sample-dir",  type=Path, default="samples_nfe100",             help="directory where samples are stored")
