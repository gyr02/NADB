...
def get_crop_params(size, crop_size):  
    w, h = size 
...
def get_transformdit(params, crop_size,  flip=True, crop = True, totensor=True):
    transform_list = []
    transform_list.append(transforms.Lambda(lambda img: __scale(img, crop_size, method)))
...
class DiTDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataroot, train=True,  img_size=512, random_crop=False, random_flip=True):
     
        super().__init__()
        if train:
            self.train_lr_dir = "/train_LR"  # get the image directory
            self.train_lr_paths = make_dataset(self.train_lr_dir) # get image paths
            self.train_hr_dir = "/train_HR"  # get the image directory
            self.train_hr_paths = make_dataset(self.train_hr_dir) # get image paths
            self.train_hr_paths = sorted(self.train_hr_paths)
            self.train_lr_paths = sorted(self.train_lr_paths)
        else:
            self.train_lr_dir = "/Test_x4/test_LR"  # get the image directory
            self.train_lr_paths = make_dataset(self.train_lr_dir) # get image paths
            self.train_hr_dir = "Test_x4/test_HR"  # get the image directory
            self.train_hr_paths = make_dataset(self.train_hr_dir) # get image paths
            self.train_hr_paths = sorted(self.train_hr_paths)
            self.train_lr_paths = sorted(self.train_lr_paths)
    ...
    def __getitem__(self, index):
        ...
        lr_path = self.train_lr_paths[index]
        hr_path = self.train_hr_paths[index]
        lr = Image.open(lr_path).convert('RGB')
        hr = Image.open(hr_path).convert('RGB')
        lr = lr.resize((776,776), Image.BICUBIC)
        ...
        lr_cropped = lr.crop(crop_box)
        hr_cropped = hr.crop(crop_box)
    
        if self.random_flip:
            if random.random() > 0.5:
                lr_cropped = lr_cropped.transpose(Image.FLIP_LEFT_RIGHT)
                hr_cropped = hr_cropped.transpose(Image.FLIP_LEFT_RIGHT)
        
      
        lr_tensor = self.transform(lr_cropped)
        hr_tensor = self.transform(hr_cropped)
        
      
        return hr_tensor, lr_tensor, index
 


    def __len__(self):
        """Return the total number of images in the dataset."""
        return len(self.train_lr_paths)

class DiTtest(torch.utils.data.Dataset):
 

    def __init__(self, dataroot, train=False,  img_size=512, random_crop=False, random_flip=False):
     
        super().__init__()
       
        self.train_lr_dir = "/home/math/data/Test_x4/test_LR"  # get the image directory
        self.train_lr_paths = make_dataset(self.train_lr_dir) # get image paths
        self.train_hr_dir = "/home/math/data/Test_x4/test_HR"  # get the image directory
        self.train_hr_paths = make_dataset(self.train_hr_dir) # get image paths
        self.train_hr_paths = sorted(self.train_hr_paths)
        self.train_lr_paths = sorted(self.train_lr_paths)
    ...
    def __getitem__(self, index):
        ...
        hr = Image.open(hr_path).convert('RGB')
        lr = lr.resize((776,776), Image.BICUBIC)
        hr = hr.resize((776,776), Image.BICUBIC)
        width, height = lr.size
        ...
        lr_cropped = lr.crop((left, top, right, bottom))
