...
from nadb import Runner, Runner1,download_ckpt
from corruption import build_corruption
from dataset import imagenet
from nadb import ckpt_util
...
def build_val_dataset(opt, log, corrupt_type):
    ...
    elif "edges2handbags" in corrupt_type or "edges2shoes" in corrupt_type:
        from aligned_dataset import EdgesDataset
        val_dataset = EdgesDataset(dataroot=opt.dataset_dir, train=False, img_size= opt.image_size,
                                random_crop=False, random_flip=False)
    elif "DiT4SR" in corrupt_type:
        print("DiTDataset")
        from aligned_dataset import DiTtest
        val_dataset = DiTtest(dataroot="/home/math/data/Test_x4/", train=False, img_size=512,
                                random_crop=False, random_flip=False)
    else:
...
def compute_batch(ckpt_opt, corrupt_type, corrupt_method, out,opt):
    if "edges2handbags" in corrupt_type or "edges2shoes" in corrupt_type:
        clean_img,corrupt_img,y = out#y is index,corrupt_img is edge
        mask = None
        x1 = corrupt_img.to(opt.device)###check it 
    elif "DiT4SR" in corrupt_type:
    ...
    return clean_img,corrupt_img, x1, mask, cond, y
...
def main(opt):
    ...
    # build runner
    if opt.use_mean==True:
        runner = Runner(ckpt_opt, log, save_opt=False)
    else:
        runner = Runner1(ckpt_opt, log, save_opt=False)
    ...
    SSIM=[]
    LPIPS=[]
    loss_fn = lpips.LPIPS(net='alex').to(opt.device)
  
    for loader_itr, out in enumerate(val_loader):
        
        clean_img,corrupt_img, x1, mask, cond, y = compute_batch(ckpt_opt, corrupt_type, corrupt_method, out,opt)

        xs, _ = runner.ddpm_sampling(clean_img,ckpt_opt, x1, mask=mask, cond=cond, clip_denoise=opt.clip_denoise, nfe=nfe, d=opt.d,verbose=opt.n_gpu_per_node==1)
        ...
        #psnr
        mse = torch.mean((((clean_img+1)/2).to(opt.device) - ((recon_img+1)/2).to(opt.device)) ** 2,dim=[1, 2, 3])
        MSE.append(mse.detach())
        #ssim
        ssim_score = ssim(((clean_img+1)/2).to(opt.device), ((recon_img+1)/2).to(opt.device),data_range=1.0,size_average=True,)
        SSIM.append(ssim_score.detach())
        #lpips
        distances = loss_fn(clean_img.to(opt.device), recon_img.to(opt.device)).mean()#### 
        LPIPS.append(distances.detach())
        ...
        dist.barrier()  
    #psnr
    MSE_tensor = torch.cat(MSE,dim=0).to(opt.device)
    gather_list = [torch.zeros_like(MSE_tensor) for _ in range(dist.get_world_size())]
    dist.all_gather(gather_list, MSE_tensor)
    all_MSE = torch.cat(gather_list, dim=0)  
    all_psnr = 10 * torch.log10(1 / (all_MSE+1e-10)) 
    psnr=all_psnr.mean()
    #ssim
    
    SSIM_tensor=torch.stack(SSIM, dim=0).to(opt.device)
    gather_list = [torch.zeros_like(SSIM_tensor) for _ in range(dist.get_world_size())]
    dist.all_gather(gather_list, SSIM_tensor)
    all_SSIM = torch.cat(gather_list, dim=0)
    global_ssim=all_SSIM.mean()  
    #lpips  
    D_tensor = torch.stack(LPIPS).to(opt.device)
    dist.all_reduce(D_tensor, op=dist.ReduceOp.SUM)
    global_lpips = D_tensor.mean() / dist.get_world_size()

    if opt.global_rank == 0:
        print(f"Global PSNR: {psnr:.3f} dB")
        print(f"Global SSIM: {global_ssim:.3f} dB")
        print(f"Global LPIPS: {global_lpips:.3f} dB")
    del runner
...
if __name__ == '__main__':
    ...
    parser.add_argument("--master-port",    type=str,   default="11368",    help="Port for master node communication")
    # data
    ...
    parser.add_argument("--d",              type=float, default=None,       help="d is within the [0,1]")
